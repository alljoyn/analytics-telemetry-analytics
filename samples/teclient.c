...
teErrType te_add_event(teUpdateState *statep, const char *name,
       int64_t timestamp, int num_keys, teKeyValue kv[])
{
    unsigned kv_length;
    ...
    if (timestamp) {
        event_length += 1 + wirelength_int64(timestamp);
    }
