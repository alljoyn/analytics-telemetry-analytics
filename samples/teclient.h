...
#include "tesite.h"
    ...
    #define TE_TIMESTAMP_TYPE int64_t
#else
    #define TE_TIMESTAMP_TYPE int32_t
#endif

struct teUpdateState;

typedef enum { TE_SUCCESS=0, TE_ERR_ALLOC=1, TE_ERR_UNKNOWN=999 } teErrType;
typedef enum { TE_STRING=2, TE_I32=3, TE_FLOAT=4, TE_DOUBLE=5, TE_I64=6
        } teDataType;
...
typedef struct teUpdateState {
    ...
    void *buf;
    int32_t buf_size ;  /* total current capacity of current buffer. */
    int32_t used ;      /* number of bytes in the update so far. */
    int hadError ;      /* Available for use by a custom teBufferManager */
} teUpdateState;
...
typedef struct teKeyValue {
    ...
    int _scratch[3] ;   /* used internally by the library. */
} teKeyValue;
